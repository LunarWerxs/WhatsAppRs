...
| `tray-test.ps1 -Engine X` | Close-to-tray, restore from a second launch, clean quit, and (for Firefox) that no orphan browser is left behind. Measured rather than assumed: both new engines put a foreign window inside ours, and one of them puts another process's window there. |
