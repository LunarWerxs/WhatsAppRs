...
The second is the ground truth and the first is not. Measured twice on two different engines:
the page reported `onshow` and "displayed" for every notification while Windows recorded none
and nothing appeared on screen. That report was once the entire basis of a "notifications work"
claim that was false.

  .\notify-test.ps1
  .\notify-test.ps1 -NoBridge     # the same run with our shim OFF, which is how you measure
                                  # what the engine does by itself: nothing at all
#>
param(
    [int]$Wait = 45,
    [switch]$NoBridge,
...
$dataDir = "$env:LOCALAPPDATA\WhatsAppRs-notify"
...
# The Start Menu shortcut carrying the AppUserModelID is what makes Windows willing to draw a
# toast for this app at all. The app writes it at startup; check it is really there.
$lnk = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\WhatsApp Rs.lnk'
"shortcut : " + $(if (Test-Path $lnk) { "present" } else { 'MISSING - no toast will ever render' })
...
$p = Start-Process -FilePath $Exe -ArgumentList '--safe' -WorkingDirectory (Split-Path $Exe) -PassThru `
        -RedirectStandardOutput "$PSScriptRoot\notify.out" -RedirectStandardError "$PSScriptRoot\notify.err"
Start-Sleep -Seconds $Wait
if ($p.HasExited) { "EXITED $($p.ExitCode)"; Get-Content "$PSScriptRoot\notify.err" -Tail 20; exit 2 }

"page says: " + (Invoke-Python @("$PSScriptRoot\cdp-eval.py", "$DebugPort", "$PSScriptRoot\notify-probe.js"))
...
$shot = "$PSScriptRoot\notify$(if ($NoBridge) { '-nobridge' }).png"
$b.Save($shot)
"screenshot: $shot"
...
"--- app stderr, last 15 ---"
Get-Content "$PSScriptRoot\notify.err" -Tail 15 -ErrorAction SilentlyContinue |
