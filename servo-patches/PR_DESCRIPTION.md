...
which would be a false statement on these. So: the branches exist locally
(`pr/bindings-keyword-escape`, `pr/indexeddb`, `pr/web-locks`, `pr/cache-storage` in the
Servo checkout beside this repo, rebased on upstream main 6c680f538 on 2026-09-07), a public
fork `LunarWerxs/servo` was created and nothing was pushed to it, and no PR or issue was opened.

What IS allowed and useful: the bugs themselves are real, measured, and reproducible on
web.whatsapp.com. `ISSUE_DRAFT.md` beside this file is a bug report a person can read, rewrite
in their own words, and file under their own name if they judge it worth Servo's time. That is
