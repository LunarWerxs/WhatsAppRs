...
- ⛔⛔ **THE ENGINE IS OS-AGNOSTIC OR IT IS NOT A CANDIDATE.** The SAME engine on Windows, macOS and Linux, never one per platform. Owner, 2026-09-09: *"I will never, ever, ever use a browser that is not OS agnostic."* **WebView2 / Microsoft Edge is PERMANENTLY CLOSED and must never be raised again** - Microsoft ships it for Windows only, so it fails before size or user agent matter. "Use the system webview" is the same failure three times over. This is why CEF is bundled and why 271 MB is an accepted price. anchors: `DECISIONS.md:1`, `src/cef_view.rs:1`
...
- The unpacked engine is compressed IN PLACE on first run with `compact /exe:LZX` (WOF, the same transparent compression Windows applies to its own system binaries): 345.9 MB becomes 156.0 MB, costs ~+0.27 s of startup and **no memory** - WOF decompresses into the SHARED file cache, so the six processes still share those pages. This is NOT UPX, which is on record crashing cefclient because a packer decompresses eagerly into private memory and destroys the loader's memory-mapping. Best effort: non-NTFS (a FAT32 stick in portable mode), old Windows, policy or a missing compact.exe all just leave it uncompressed. Must run BEFORE the DLLs are loaded, since compact cannot touch a mapped file. anchors: `src/engine.rs:129`, `DECISIONS.md:472`
- ⛔ THE HUNT FOR A SMALLER ENGINE IS CLOSED, by measurement rather than opinion. No smaller modern libcef.dll exists publicly (80 GitHub repos swept, every release, every asset, 0 failed calls; the one 50.9 MB hit is a 2015-era Chromium with ZERO `WebAssembly` strings, so it cannot render the site). Stripping Google's services is worth 0.2 MB (ungoogled-chromium's chrome.dll vs stock Chrome, same branch, x64). CEF does NOT forbid feature-stripping - an earlier claim in this project that it did was WRONG, `gn_args.py` passes GN_DEFINES straight through and only DRM is forced - the blocker is the cost of owning a from-source build. Do not re-run this search; read the numbers. anchors: `FINDINGS.md:1399`, `DECISIONS.md:501`
